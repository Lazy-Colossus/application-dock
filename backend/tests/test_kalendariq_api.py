"""Story 1.3 — the first Kalendariq endpoints.
...
from app.repositories import kalendariq_repo
...
def isolate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("app.core.config.settings.kalendariq_guests", "players")
    return tmp_path
...
@pytest.mark.parametrize(
    "method,path",
    [
        ("get", "/api/kalendariq/me"),
        ("get", "/api/kalendariq/calendars"),
        ("post", "/api/kalendariq/calendars"),
        ("get", "/api/kalendariq/calendars/cal-ab12cd34"),
    ],
)
...
def test_me_reports_an_admin() -> None:
    body = client.get("/api/kalendariq/me").json()
...
def test_me_carries_the_server_date(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.services.kalendariq_service.today", lambda: date(2026, 9, 3))
    assert client.get("/api/kalendariq/me").json()["today"] == "2026-09-03"
...
def test_a_guest_cannot_create_and_nothing_is_written(as_guest: None) -> None:
    ...
    app.dependency_overrides[get_current_user] = lambda: "test_user"
    assert client.get("/api/kalendariq/calendars").json() == []
...
def test_list_returns_summaries_newest_first(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.services.kalendariq_service.now_iso", lambda: next(stamps))

    for name in ("First", "Second", "Third"):
        create(name, ["Dani"])

    body = client.get("/api/kalendariq/calendars").json()
...
def test_list_carries_the_invitee_count() -> None:
    ...
    assert client.get("/api/kalendariq/calendars").json()[0]["invitee_count"] == 3
...
def test_a_guest_sees_the_same_list_as_an_admin(as_guest: None) -> None:
    ...
    assert [c["name"] for c in client.get("/api/kalendariq/calendars").json()] == ["Shared"]
...
def test_get_returns_the_full_calendar() -> None:
    created = create().json()
    fetched = client.get(f"/api/kalendariq/calendars/{created['id']}").json()
    assert fetched == created


def test_get_of_an_unknown_calendar_is_404() -> None:
    response = client.get("/api/kalendariq/calendars/cal-nope1234")
...
def test_get_of_a_malformed_id_is_never_a_server_error(bad: str) -> None:
    response = client.get(f"/api/kalendariq/calendars/{bad}")
...
def test_list_ordering_is_deterministic_within_one_second() -> None:
    ...
    first = [c["id"] for c in client.get("/api/kalendariq/calendars").json()]
    second = [c["id"] for c in client.get("/api/kalendariq/calendars").json()]
    assert first == second
    assert len(first) == 4
...
def test_rename_changes_only_the_name(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.services.kalendariq_service.now_iso", lambda: "2026-12-25T12:00:00Z")

    renamed = client.put(
        f"/api/kalendariq/calendars/{created['id']}", json={"name": "DnD — Strahd"}
    ).json()
...
def test_rename_trims_surrounding_whitespace() -> None:
    created = create().json()
    renamed = client.put(
        f"/api/kalendariq/calendars/{created['id']}", json={"name": "  Movie night  "}
    ).json()
...
def test_rename_persists() -> None:
    created = create().json()
    client.put(f"/api/kalendariq/calendars/{created['id']}", json={"name": "Renamed"})
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").json()["name"] == "Renamed"
    assert [c["name"] for c in client.get("/api/kalendariq/calendars").json()] == ["Renamed"]


@pytest.mark.parametrize("bad", ["", "   ", "\t"])
def test_rename_rejects_a_blank_name(bad: str) -> None:
    created = create("Keep me").json()
    response = client.put(f"/api/kalendariq/calendars/{created['id']}", json={"name": bad})

    assert response.status_code == 422
    assert "detail" in response.json()
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").json()["name"] == "Keep me"


def test_rename_of_an_unknown_calendar_is_404() -> None:
    response = client.put("/api/kalendariq/calendars/cal-nope1234", json={"name": "X"})
    assert response.status_code == 404
...
def test_a_guest_cannot_rename(as_guest: None) -> None:
    ...
    response = client.put(f"/api/kalendariq/calendars/{created['id']}", json={"name": "Hijacked"})

    assert response.status_code == 403
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").json()["name"] == "Untouched"
...
def test_delete_removes_the_calendar() -> None:
    ...
    assert client.delete(f"/api/kalendariq/calendars/{created['id']}").status_code == 204
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").status_code == 404
    assert client.get("/api/kalendariq/calendars").json() == []
...
def test_delete_leaves_other_calendars_alone() -> None:
    ...
    client.delete(f"/api/kalendariq/calendars/{drop['id']}")

    assert [c["id"] for c in client.get("/api/kalendariq/calendars").json()] == [keep["id"]]


def test_delete_of_an_unknown_calendar_is_404() -> None:
    assert client.delete("/api/kalendariq/calendars/cal-nope1234").status_code == 404
...
def test_a_guest_cannot_delete(as_guest: None) -> None:
    ...
    response = client.delete(f"/api/kalendariq/calendars/{created['id']}")

    assert response.status_code == 403
    app.dependency_overrides[get_current_user] = lambda: "test_user"
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").status_code == 200


@pytest.mark.parametrize("bad", ["..", " "])
def test_mutating_a_malformed_id_is_never_a_server_error(bad: str) -> None:
    assert client.put(f"/api/kalendariq/calendars/{bad}", json={"name": "X"}).status_code in (
        404,
        422,
    )
    assert client.delete(f"/api/kalendariq/calendars/{bad}").status_code in (404, 422)
...
def add_invitee(calendar_id: str, name: str):
    return client.post(f"/api/kalendariq/calendars/{calendar_id}/invitees", json={"name": name})
...
def test_add_invitee_trims_and_persists() -> None:
    ...
    stored = client.get(f"/api/kalendariq/calendars/{created['id']}").json()
...
def seed_votes(calendar_id: str, votes: dict) -> None:
    ...
    calendar = kalendariq_repo.read_calendar(calendar_id)
    calendar.votes = votes
    kalendariq_repo.write_calendar(calendar)
...
def test_removal_clears_the_future_and_keeps_the_past(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    body = client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{dani}").json()
...
def test_a_removed_invitee_is_tombstoned_with_their_name(
    ...
    body = client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{dani['id']}").json()
...
def test_an_invitee_with_no_past_votes_is_dropped_outright() -> None:
    ...
    body = client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{oops}").json()
...
def test_removal_never_touches_chosen_dates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    calendar = kalendariq_repo.read_calendar(created["id"])
...
def test_a_tombstone_leaves_the_roster(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    dani = created["invitees"][0]
    seed_votes(created["id"], {"2026-08-10": {dani["id"]: "yes"}})
    client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{dani['id']}")

    assert client.get("/api/kalendariq/calendars").json()[0]["invitee_count"] == 0
...
def test_removing_an_already_removed_or_unknown_invitee_is_404(
    ...
    client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{dani}")
    assert (
        client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{dani}").status_code
        == 404
    )
    assert (
        client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/inv-nope").status_code
        == 404
    )
...
def test_summary_lists_invitee_names_in_roster_order() -> None:
    create("DnD", ["Dani", "Jake", "Tom"])
    summary = client.get("/api/kalendariq/calendars").json()[0]
...
def test_summary_excludes_tombstoned_invitees(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client.delete(f"/api/kalendariq/calendars/{created['id']}/invitees/{departing}")
...
def test_summary_names_survive_a_rename_of_the_calendar() -> None:
    created = create("DnD", ["Dani"]).json()
    client.put(f"/api/kalendariq/calendars/{created['id']}", json={"name": "Strahd"})
...
def vote(calendar_id: str, invitee_id: str, day: str, status: str):
    return client.put(
        f"/api/kalendariq/calendars/{calendar_id}/votes",
...
def test_a_past_day_is_refused(fixed_today) -> None:
    ...
    assert response.status_code == 422
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").json()["votes"] == {}
...
def set_chosen(calendar_id: str, day: str, chosen: bool):
    return client.put(
        f"/api/kalendariq/calendars/{calendar_id}/chosen",
...
def test_a_guest_cannot_mark_a_day(as_guest: None, fixed_today) -> None:
    ...
    app.dependency_overrides[get_current_user] = lambda: "test_user"
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").json()["chosen_dates"] == []
...
def vote_bulk(
    ...
    return client.put(
        f"/api/kalendariq/calendars/{calendar_id}/votes/bulk",
        json={
...
def set_note(calendar_id: str, invitee_id: str, day: str, text: str):
    return client.put(
        f"/api/kalendariq/calendars/{calendar_id}/notes",
...
def test_a_note_that_is_too_long_is_refused(fixed_today) -> None:
    from app.schemas.kalendariq import NOTE_MAX_LENGTH
...
def test_a_calendar_made_before_links_existed_gets_one_on_first_read(fixed_today) -> None:
    created = create("DnD", ["Dani"]).json()
    path = Path(kalendariq_repo._calendar_path(created["id"]))
    stored = json.loads(path.read_text())
    del stored["share_token"]
    path.write_text(json.dumps(stored))

    first = client.get(f"/api/kalendariq/calendars/{created['id']}").json()["share_token"]
    assert first

    # Minted once and kept: a fresh token per read would break every link
    # already pasted into the group chat.
    second = client.get(f"/api/kalendariq/calendars/{created['id']}").json()["share_token"]
    assert second == first


def shared(token: str):
    return client.get(f"/api/kalendariq/share/{token}")
...
def test_the_link_opens_the_calendar_without_logging_in(fixed_today) -> None:
    ...
    # The page cannot ask /kalendariq/me for the date on this path (NFR-5).
...
def test_the_link_can_vote(fixed_today) -> None:
    ...
    body = client.put(
        f"/api/kalendariq/share/{created['share_token']}/votes",
...
def test_the_link_reaches_nothing_but_those_four_verbs(fixed_today) -> None:
    ...
    forbidden = [
        client.patch(f"/api/kalendariq/share/{token}", json={"name": "Renamed"}),
        client.delete(f"/api/kalendariq/share/{token}"),
        client.post(f"/api/kalendariq/share/{token}/invitees", json={"name": "Tom"}),
        client.put(
            f"/api/kalendariq/share/{token}/chosen",
            json={"date": "2026-09-14", "chosen": True},
        ),
        client.get("/api/kalendariq/share"),
    ]
...
def test_the_link_needs_no_login_at_all(fixed_today, monkeypatch) -> None:
    ...
    assert client.get(f"/api/kalendariq/calendars/{created['id']}").status_code == 401
    assert client.get("/api/kalendariq/calendars").status_code == 401

    assert shared(token).status_code == 200
    assert (
        client.put(
            f"/api/kalendariq/share/{token}/votes",
            json={"invitee_id": dani, "date": "2026-09-14", "status": "yes"},
        ).status_code
        == 200
    )
