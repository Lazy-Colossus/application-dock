...
def make(name: str = "Dal", body: str = "Simmer the lentils.", **extra: object):
    return service.create_recipe(name=name, body=body, **extra)  # type: ignore[arg-type]
...
def test_a_mark_survives_a_read_write_round_trip() -> None:
    recipe = make()
    service.apply_enrichment({recipe.id: {"source": "a magazine"}}, write=True)

    reread = service.get_recipe(recipe.id)
...
def test_a_user_edit_marks_the_field_user_set_whatever_it_was() -> None:
    recipe = make()
    service.apply_enrichment({recipe.id: {"source": "guessed"}}, write=True)
    assert service.get_recipe(recipe.id).unconfirmed == ["source"]

    service.update_recipe(recipe.id, {"source": "the actual book"})

    # The mark is gone: the user's value landed on top of the guess.
    assert service.get_recipe(recipe.id).unconfirmed == []


def test_a_mark_for_a_value_that_is_gone_is_dropped() -> None:
    recipe = make()
    service.apply_enrichment({recipe.id: {"tags": ["guessed"]}}, write=True)
    assert service.get_recipe(recipe.id).unconfirmed == ["tag:guessed"]

    service.update_recipe(recipe.id, {"tags": []})
...
def test_recipes_predating_enrichment_default_to_user_set(isolate: Path) -> None:
    ...
    assert service.get_recipe("r-old").unconfirmed == []
...
def test_the_report_names_what_is_missing() -> None:
    bare = make(name="Bare")
    report = service.enrichment_report()
...
def test_a_fully_structured_recipe_is_left_out_of_the_report() -> None:
    ...
    assert service.enrichment_report()["needing_attention"] == 0


def test_the_report_writes_nothing(isolate: Path) -> None:
    make()
    before = (isolate / "kitchencraft" / "recipes.json").read_text()
    service.enrichment_report()
    assert (isolate / "kitchencraft" / "recipes.json").read_text() == before


def test_a_dry_run_changes_nothing_but_reports_what_it_would() -> None:
    recipe = make()
    result = service.apply_enrichment({recipe.id: {"source": "guessed"}})

    assert result["write"] is False
    assert result["applied"] == [recipe.id]
    assert service.get_recipe(recipe.id).source is None
...
def test_every_body_is_byte_identical_afterwards() -> None:
    ...
    service.apply_enrichment({recipe.id: {"meal_type": "dinner", "tags": ["x"]}}, write=True)
    assert service.get_recipe(recipe.id).body == body


def test_a_batch_lands_after_the_user_has_renamed_the_recipe() -> None:
    recipe = make(name="Original")
    service.update_recipe(recipe.id, {"name": "Renamed since"})

    service.apply_enrichment({recipe.id: {"source": "a book"}}, write=True)
...
def test_an_unknown_id_is_reported_and_the_rest_still_applies() -> None:
    ...
    assert result["applied"] == [recipe.id]
    assert service.get_recipe(recipe.id).source == "a book"
...
def test_an_identical_batch_run_twice_is_a_no_op() -> None:
    ...
    service.apply_enrichment(batch, write=True)
    first = service.get_recipe(recipe.id)

    second_result = service.apply_enrichment(batch, write=True)
    second = service.get_recipe(recipe.id)
...
def test_a_user_set_field_is_left_alone_and_the_skip_is_reported() -> None:
    ...
    result = service.apply_enrichment({recipe.id: {"meal_type": "dinner"}}, write=True)

    assert result["skipped"] == {recipe.id: ["meal_type"]}
    assert service.get_recipe(recipe.id).meal_type == "breakfast"
...
def test_a_machine_set_field_may_be_revised_by_a_later_pass() -> None:
    ...
    recipe = make()
    service.apply_enrichment({recipe.id: {"servings": 2}}, write=True)
    service.apply_enrichment({recipe.id: {"servings": 6}}, write=True)

    assert service.get_recipe(recipe.id).servings == 6
...
def test_a_hand_written_ingredient_list_is_never_added_to() -> None:
    ...
    result = service.apply_enrichment({recipe.id: {"ingredients": [{"text": "cumin"}]}}, write=True)

    assert result["skipped"] == {recipe.id: ["ingredients"]}
    assert [i.text for i in service.get_recipe(recipe.id).ingredients] == ["red lentils"]
...
def test_an_invalid_number_is_rejected_and_nothing_is_written() -> None:
    ...
    with pytest.raises(ValueError):
        service.apply_enrichment({recipe.id: {"servings": 0}}, write=True)

    # The transaction did not complete, so no partial mutation survives.
    assert service.get_recipe(recipe.id).servings is None


def test_the_rating_is_never_written_by_enrichment() -> None:
    # A judgement only the cook can make (Story 2.7).
    recipe = make()
    service.apply_enrichment({recipe.id: {"rating": 5}}, write=True)
    assert service.get_recipe(recipe.id).rating is None
...
def test_a_pass_reuses_the_casing_already_in_the_collection() -> None:
    ...
    service.apply_enrichment({second.id: {"tags": ["batch-cook"]}}, write=True)

    # One tag in the collection, not two that differ only in casing.
    assert service.get_vocabulary().tags == ["Batch-Cook"]
    # Re-read: the first recipe's own tag is untouched by the second's pass.
    assert service.get_recipe(first.id).tags == ["Batch-Cook"]
...
def test_an_ingredient_folds_onto_the_casing_the_collection_uses() -> None:
    ...
    service.apply_enrichment({second.id: {"ingredients": [{"text": "Red Lentils"}]}}, write=True)

    assert service.get_vocabulary().ingredients == ["red lentils"]


def test_a_tag_already_on_the_recipe_is_not_duplicated_or_marked() -> None:
    recipe = make(tags=["weeknight"])
    service.apply_enrichment({recipe.id: {"tags": ["Weeknight"]}}, write=True)
...
def test_new_wording_is_reported_so_drift_is_visible(
    ...
    enrich.main(["apply", "--batch", str(batch), "--write"])
    ...
    assert service.get_recipe(recipe.id).tags == ["weeknight"]
...
def test_the_script_exits_non_zero_when_an_id_was_unknown(isolate: Path) -> None:
    ...
    assert enrich.main(["apply", "--batch", str(batch)]) == 1
...
def test_the_script_writes_nothing_without_the_flag(isolate: Path) -> None:
    ...
    assert enrich.main(["apply", "--batch", str(batch)]) == 0
    assert service.get_recipe(recipe.id).source is None
...
def test_the_script_writes_with_the_flag(isolate: Path) -> None:
    ...
    assert enrich.main(["apply", "--batch", str(batch), "--write"]) == 0
    assert service.get_recipe(recipe.id).source == "guessed"
...
def test_the_script_rejects_a_batch_that_is_not_an_object(isolate: Path) -> None:
    ...
    assert enrich.main(["apply", "--batch", str(batch)]) == 2
