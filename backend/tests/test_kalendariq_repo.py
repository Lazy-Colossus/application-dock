"""Story 1.2 — the Kalendariq data layer.

Kalendariq's storage is keyed by calendar id, not by username: the group shares one
...
from app.schemas.kalendariq import Calendar, Invitee
...
def test_first_write_creates_the_calendars_directory(isolate: Path) -> None:
    assert not (isolate / "kalendariq" / "calendars").exists()
    repo.write_calendar(make_calendar())
    assert (isolate / "kalendariq" / "calendars" / "cal-ab12cd34.json").is_file()
...
def test_list_ignores_non_json_files(isolate: Path) -> None:
    repo.write_calendar(make_calendar())
    stray = isolate / "kalendariq" / "calendars" / "notes.txt"
...
def test_stored_document_shape_matches_the_spec(isolate: Path) -> None:
    ...
    raw = json.loads((isolate / "kalendariq" / "calendars" / "cal-ab12cd34.json").read_text())
...
class TestLegacyAppDirMigration:
    """Data written when the app was called KDH must survive the rename."""

    def test_calendars_under_the_old_name_are_found(self, isolate: Path) -> None:
        calendar = make_calendar()
        legacy = isolate / "kdh" / "calendars"
        legacy.mkdir(parents=True)
        (legacy / f"{calendar.id}.json").write_text(calendar.model_dump_json(), encoding="utf-8")

        assert [c.id for c in repo.list_calendars()] == [calendar.id]
        assert not (isolate / "kdh").exists()

    def test_migration_is_idempotent(self, isolate: Path) -> None:
        (isolate / "kdh" / "calendars").mkdir(parents=True)

        repo.list_calendars()
        repo.list_calendars()

        assert (isolate / "kalendariq" / "calendars").is_dir()

    def test_two_trees_are_left_alone_rather_than_merged(self, isolate: Path) -> None:
        legacy = make_calendar(calendar_id="cal-old00000")
        current = make_calendar(calendar_id="cal-new00000")
        for app_dir, cal in (("kdh", legacy), ("kalendariq", current)):
            directory = isolate / app_dir / "calendars"
            directory.mkdir(parents=True)
            (directory / f"{cal.id}.json").write_text(cal.model_dump_json(), encoding="utf-8")

        assert [c.id for c in repo.list_calendars()] == [current.id]
        assert (isolate / "kdh" / "calendars" / f"{legacy.id}.json").exists()
