"""Process-local pub/sub for live note events (Story 2.2).

A member's write to a note publishes a coarse event; every other member
subscribed to that note refetches and reconverges. Events are keyed by
`note_id`.
...
This mirrors `listies_events` deliberately rather than sharing code with it: a
generic bus belongs in `core/` once a third app needs one, which is the same
threshold `core/storage.py` was extracted at.
"""
...
# note_id → { queue: meta }. Guarded by `_lock` because publishers and
# subscribers run on different threads.
_subs: dict[str, dict[asyncio.Queue[Event], _Meta]] = {}
_lock = threading.Lock()


def subscribe(note_id: str) -> asyncio.Queue[Event]:
    """Register interest in a note's events; returns the subscriber's queue.
    ...
    with _lock:
        _subs.setdefault(note_id, {})[queue] = meta
    return queue


def unsubscribe(note_id: str, queue: asyncio.Queue[Event]) -> None:
    """Remove a subscriber. Safe to call more than once."""
    with _lock:
        subs = _subs.get(note_id)
...
def publish(note_id: str, event: Event) -> None:
    """Fan an event out to every subscriber of `note_id`.
    ...
    """
    with _lock:
        targets = list(_subs.get(note_id, {}).items())
...
def is_stale(note_id: str, queue: asyncio.Queue[Event]) -> bool:
    """Whether this subscriber overflowed and should be dropped (it will resync)."""
    with _lock:
        meta = _subs.get(note_id, {}).get(queue)
        return meta.stale if meta is not None else True


def subscriber_count(note_id: str) -> int:
    """Number of live subscribers for a note (for tests/introspection)."""
    with _lock:
        return len(_subs.get(note_id, {}))
