...
describe("useClaimedName", () => {
  ...
  it("claims a name and remembers it per calendar", () => {
    ...
    expect(window.localStorage.getItem("kalendariq.claim.cal-1")).toBe("inv-1");
    expect(window.localStorage.getItem("kalendariq.claim.cal-2")).toBeNull();
  });

  it("restores a stored claim on a later visit", () => {
    window.localStorage.setItem("kalendariq.claim.cal-1", "inv-1");
    const c = useClaimedName(
      () => "cal-1",
      () => [DANI],
    );
  ...
  it("releases a claim", () => {
    ...
    expect(c.hasClaim.value).toBe(false);
    expect(window.localStorage.getItem("kalendariq.claim.cal-1")).toBeNull();
  });

  it("discards a claim naming someone since removed", async () => {
    window.localStorage.setItem("kalendariq.claim.cal-1", GONE.id);
    const c = useClaimedName(
      () => "cal-1",
      () => [DANI, GONE],
    );
  ...
  it("discards a claim naming nobody at all", async () => {
    window.localStorage.setItem("kalendariq.claim.cal-1", "inv-nonsense");
    const c = useClaimedName(
      () => "cal-1",
      () => [DANI],
    );
