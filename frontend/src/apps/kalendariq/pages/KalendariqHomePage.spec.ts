...
import KalendariqHomePage from "./KalendariqHomePage.vue";
...
async function mountPage() {
  const wrapper = mount(KalendariqHomePage, MOUNT_OPTS);
  await flushPromises();
  return wrapper;
}

describe("KalendariqHomePage", () => {
  beforeEach(() => {
  ...
  it("opens a calendar on selection", async () => {
    ...
    expect(push).toHaveBeenCalledWith("/kalendariq/c/cal-1");
  });
  ...
  it("creates a calendar and navigates to it", async () => {
    ...
    expect(postMock).toHaveBeenCalledWith("/kalendariq/calendars", {
      name: "Birthday",
      invitee_names: ["Dani"],
    });
    expect(push).toHaveBeenCalledWith("/kalendariq/c/cal-new");
  });
  ...
  describe("the invitee link on a row", () => {
    ...
    it("copies the link an invitee follows, not this page's address", async () => {
      ...
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
        `${window.location.origin}/kalendariq/s/tok-dnd`,
      );
    ...
    it("copies the right row's link", async () => {
      ...
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
        `${window.location.origin}/kalendariq/s/tok-movie`,
      );
    });
