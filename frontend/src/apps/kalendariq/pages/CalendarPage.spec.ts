...
const { routeRef } = vi.hoisted(() => ({
  routeRef: {
    current: {
      ...
      path: "/kalendariq/c/cal-ab12cd34",
    },
  },
}));
vi.mock("vue-router", () => ({
...
import type { Calendar } from "@/apps/kalendariq/types";
...
describe("CalendarPage", () => {
  ...
  it("goes back to the list", async () => {
    ...
    expect(push).toHaveBeenCalledWith("/kalendariq");
  });
  ...
  it("renames the calendar in place", async () => {
    ...
    expect(putMock).toHaveBeenCalledWith("/kalendariq/calendars/cal-ab12cd34", {
      name: "Strahd",
    });
  ...
  it("names the calendar in the delete confirmation before deleting", async () => {
    ...
    expect(delMock).toHaveBeenCalledWith("/kalendariq/calendars/cal-ab12cd34");
    expect(push).toHaveBeenCalledWith("/kalendariq");
  });
  ...
  it("copies the invitee link, not the admin address bar", async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      `${window.location.origin}/kalendariq/s/tok-secret`,
    );
  ...
  it("lists the roster and adds an invitee", async () => {
    ...
    expect(postMock).toHaveBeenCalledWith(
      "/kalendariq/calendars/cal-ab12cd34/invitees",
      {
        name: "Jake",
      },
    );
  ...
  it("warns what is kept before removing an invitee", async () => {
    ...
    expect(delMock).toHaveBeenCalledWith(
      "/kalendariq/calendars/cal-ab12cd34/invitees/inv-1",
    );
  ...
  it("restores a stored claim on a later visit", async () => {
    window.localStorage.setItem("kalendariq.claim.cal-ab12cd34", "inv-1");
    mockApi(false, { ...CAL, invitees: TWO_INVITEES });
    const wrapper = await mountPage();
  ...
  it("discards a stored claim naming someone since removed", async () => {
    window.localStorage.setItem("kalendariq.claim.cal-ab12cd34", "inv-gone");
    mockApi(false, { ...CAL, invitees: TWO_INVITEES });
    const wrapper = await mountPage();
  ...
  it("does not open a past day at all", async () => {
    ...
    expect(past.find(".kalendariq-voter").text()).toContain("Jake");
  });
  ...
  it("collects days instead of opening them, and marks them all free", async () => {
    ...
    expect(putMock).toHaveBeenCalledWith(
      "/kalendariq/calendars/cal-ab12cd34/votes/bulk",
      {
        invitee_id: "inv-1",
  ...
  describe("opened from an invitee link", () => {
    ...
    async function mountShared(
      ...
      routeRef.current = {
        ...
        path: "/kalendariq/s/tok-secret",
      };
      getMock.mockImplementation((path: string) => {
        if (path === "/kalendariq/share/tok-secret")
    ...
    it("asks once, unauthenticated, and never for /kalendariq/me", async () => {
      await mountShared();
