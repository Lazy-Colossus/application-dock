...
import type {
  ...
  Me,
  VoteStatus,
} from "@/apps/kalendariq/types";

export const useKalendariqStore = defineStore("kalendariq", () => {
  ...
  function writeBase(calendarId: string): string {
    return shareToken.value
      ? `/kalendariq/share/${shareToken.value}`
   ...
   *
   * The response carries the server's date because there is no `/kalendariq/me` to ask
  ...
  async function fetchMe(): Promise<void> {
    ...
    error.value = null;
    try {
      me.value = await api.get<Me>("/kalendariq/me");
    } catch (e) {
      error.value = e instanceof Error ? e.message : String(e);
    } finally {
      loading.value = false;
  ...
  async function deleteCalendar(calendarId: string): Promise<void> {
    ...
    try {
      ...
      await api.del(`/kalendariq/calendars/${calendarId}`);
  ...
  async function addInvitee(calendarId: string, name: string): Promise<void> {
    ...
    try {
      const updated = await api.post<Calendar>(
        `/kalendariq/calendars/${calendarId}/invitees`,
        { name },
      );
      currentCalendar.value = updated;
      syncSummary(updated);
  ...
  async function removeInvitee(
    ...
    try {
      const updated = await api.del<Calendar>(
        `/kalendariq/calendars/${calendarId}/invitees/${inviteeId}`,
      );
      currentCalendar.value = updated;
      syncSummary(updated);
    } catch (e) {
  ...
  async function setChosen(
    ...
    try {
      const updated = await api.put<Calendar>(
        `/kalendariq/calendars/${calendarId}/chosen`,
        { date, chosen },
      );
      currentCalendar.value = updated;
  ...
  async function createCalendar(
    ...
    error.value = null;
    try {
      const created = await api.post<Calendar>("/kalendariq/calendars", {
        name,
