...
const SESSION = {
  ...
  targets: [
    ...
    },
    {
      number: 5,
      scores: {
        Alice: [5, 0] as [number, number],
        Bob: [10, 11] as [number, number],
      },
      confirmed: true,
    },
  ],
  ...
  return mount(ScoringBoardPage, {
    global: {
      ...
      stubs: {
        ...
        TargetIcon: {
          template:
            '<button class="target-icon-stub" :data-confirmed="confirmed" @click="$emit(\'tap\', number)">{{ number }}</button>',
...
describe("ScoringBoardPage", () => {
  ...
  it("shows a bottom-anchored View Results control (Story 7.5)", () => {
    const w = mountPage();
    expect(w.find('[data-testid="view-results-bottom-btn"]').exists()).toBe(
      true,
    );
  });
