...
const STUBS = {
  ...
  "q-input": {
    template:
      '<div><input :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /><div v-if="errorMessage" data-testid="input-error">{{ errorMessage }}</div></div>',
  ...
  ArcherChip: {
    template:
      '<div class="archer-chip">{{ name }}<button @click="$emit(\'remove\')">x</button></div>',
...
describe("SessionSetupPage", () => {
  ...
  it("shows error when adding empty name", async () => {
    ...
    expect(wrapper.find('[data-testid="input-error"]').text()).toBe(
      "Archer name is required.",
    );
  });
...
describe("SessionSetupPage — recurring player picker (Story 8.4)", () => {
  ...
  it("calls loadPlayers on mount", async () => {
    ...
    const recurringStore = useRecurringPlayersStore();
    const spy = vi
      .spyOn(recurringStore, "loadPlayers")
      .mockResolvedValue(undefined);
    mount(SessionSetupPage, {
      global: { plugins: [pinia, router], stubs: STUBS },
    });
    await flushPromises();
  ...
  it("picker is hidden by default", () => {
    ...
    expect(wrapper.find('[data-testid="recurring-picker"]').exists()).toBe(
      false,
    );
  });
  ...
  it("selecting a player closes the picker (AC 3)", async () => {
    ...
    await wrapper.vm.$nextTick();
    await wrapper
      .find('[data-testid="recurring-player-option"]')
      .trigger("click");
    await wrapper.vm.$nextTick();
    expect(wrapper.find('[data-testid="recurring-picker"]').exists()).toBe(
      false,
  ...
  it("already-rostered player cannot be re-added via picker (AC 4)", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(
      wrapper.findAll('[data-testid="recurring-player-option"]').length,
    ).toBe(0);
  });
  ...
  it("shows empty state pointing to management when no recurring players exist (AC 5)", async () => {
    ...
    await wrapper.find('[data-testid="picker-btn"]').trigger("click");
    await wrapper.vm.$nextTick();
    expect(wrapper.find('[data-testid="picker-empty-state"]').exists()).toBe(
      true,
    );
  ...
  it("shows all-rostered message when all recurring players are already in the roster", async () => {
    ...
    await wrapper.find('[data-testid="picker-btn"]').trigger("click");
    await wrapper.vm.$nextTick();
    expect(wrapper.find('[data-testid="picker-all-rostered"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.findAll('[data-testid="recurring-player-option"]').length,
  ...
  it("picked player is excluded when picker is re-opened (AC 4)", async () => {
    ...
    await wrapper.vm.$nextTick();
    await wrapper
      .findAll('[data-testid="recurring-player-option"]')[0]
      .trigger("click");
    await wrapper.vm.$nextTick();
    await wrapper.find('[data-testid="picker-btn"]').trigger("click");
    await wrapper.vm.$nextTick();
