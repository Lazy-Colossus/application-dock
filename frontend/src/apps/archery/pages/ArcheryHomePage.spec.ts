...
function mountPage(list: InProgressSummary[] = []) {
  ...
  const wrapper = mount(ArcheryHomePage, {
...
describe("ArcheryHomePage", () => {
  ...
  it("renders Manage Players button and navigates to /archery/players", async () => {
    ...
    const { wrapper } = mountPage([]);
    expect(wrapper.find('[data-testid="manage-players-btn"]').exists()).toBe(
      true,
    );
  ...
  it("New Session with open sessions opens the conflict dialog", async () => {
    ...
    await wrapper.find('[data-testid="new-session-btn"]').trigger("click");
    await nextTick();
    expect(wrapper.find('[data-testid="conflict-leave-btn"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="conflict-delete-btn"]').exists()).toBe(
      true,
    );
  });
  ...
  it('conflict "Leave" navigates to setup without deleting', async () => {
    ...
    const discardSpy = vi
      .spyOn(store, "discardAllInProgress")
