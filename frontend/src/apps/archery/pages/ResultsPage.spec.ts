...
const STUBS = {
  ...
  ResultsTable: {
    template: '<div class="results-table-stub" />',
    props: ["session"],
  },
};
...
describe("ResultsPage", () => {
  ...
  it("finalise button opens confirm dialog", async () => {
    ...
    expect(wrapper.find('[data-testid="finalise-confirm-btn"]').exists()).toBe(
      true,
    );
  });
  ...
  it("confirming finalise calls store.finaliseSession and navigates to /archery", async () => {
    ...
    const finaliseSpy = vi
      .spyOn(store, "finaliseSession")
  ...
  it("tie-break: alphabetical when totals equal", () => {
    const tied: SessionData = {
      ...
      targets: Array.from({ length: 18 }, (_, i) => ({
        number: i + 1,
        scores: {
          Zara: [5, 5] as [number, number],
          Alice: [5, 5] as [number, number],
        },
      })),
    };
