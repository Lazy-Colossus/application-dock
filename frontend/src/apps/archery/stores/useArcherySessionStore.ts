...
import type {
  ...
  SessionData,
  TargetScores,
} from "@/apps/archery/types";

function upsertTarget(
  targets: TargetScores[],
  t: TargetScores,
): TargetScores[] {
  return [...targets.filter((x) => x.number !== t.number), t].sort(
...
export const useArcherySessionStore = defineStore("archerySession", () => {
  ...
  function isConfirmed(n: number): boolean {
    return (
      session.value?.targets.some(
        (t) => t.number === n && t.confirmed === true,
      ) ?? false
    );
  }
  ...
  async function finaliseSession(): Promise<void> {
    ...
    try {
      await api.post(
        `/archery/sessions/in-progress/${session.value.label}/finalise`,
      );
      session.value = null;
    } catch (e) {
      error.value = messageFrom(e);
  ...
  async function saveTarget(target: TargetScores): Promise<void> {
    ...
    error.value = null;
    try {
      const saved = await api.put<SessionData>(
        `/archery/sessions/in-progress/${next.label}`,
  ...
  async function loadInProgress(): Promise<void> {
    ...
    error.value = null;
    try {
      inProgressList.value = await api.get<InProgressSummary[]>(
        "/archery/sessions/in-progress",
      );
    } catch (e) {
      error.value = messageFrom(e);
    } finally {
  ...
  async function resumeSession(label: string): Promise<void> {
    ...
    error.value = null;
    try {
      session.value = await api.get<SessionData>(
        `/archery/sessions/in-progress/${label}`,
      );
    } catch (e) {
      error.value = messageFrom(e);
      throw e;
  ...
  async function discardAllInProgress(): Promise<void> {
    ...
    try {
      for (const s of [...inProgressList.value]) {
        ...
        inProgressList.value = inProgressList.value.filter(
          (x) => x.label !== s.label,
        );
      }
      session.value = null;
