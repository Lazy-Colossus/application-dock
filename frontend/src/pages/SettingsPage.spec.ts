...
const STUBS = {
  ...
  'q-input': {
    template: '<input :type="type || \'text\'" :value="modelValue" :data-has-error="error || undefined" @input="$emit(\'update:modelValue\', $event.target.value)" />',
    props: ['modelValue', 'type', 'label', 'error', 'errorMessage', 'rules', 'hint'],
...
describe('SettingsPage', () => {
  ...
  it('Add user success: POST called, username in list, Notify shown', async () => {
    ...
    await flushPromises();
    expect(vi.mocked(api.post)).toHaveBeenCalledWith('/auth/users', { username: 'alice' });
    expect(wrapper.text()).toContain('alice');
  ...
  it('Add user 409: negative Notify shown, input not cleared', async () => {
    vi.mocked(api.post).mockRejectedValue(new ApiError(409, 'Username already exists'));
    const wrapper = await mountResolved(false);
    await wrapper.find('[data-testid="new-username-input"]').setValue('duplicate');
    await wrapper.find('[data-testid="add-user-btn"]').trigger('click');
    await flushPromises();
    expect(vi.mocked(Notify.create)).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'negative' }),
    );
    expect((wrapper.find('[data-testid="new-username-input"]').element as HTMLInputElement).value).toBe('duplicate');
  });
  ...
  it('Mismatch between new and confirm shows inline error, no POST sent', async () => {
    ...
    await wrapper.find('[data-testid="confirm-password"]').setValue('different');
    await wrapper.find('[data-testid="change-password-btn"]').trigger('click');
    await flushPromises();
    expect(vi.mocked(api.post)).not.toHaveBeenCalled();
    expect(wrapper.find('[data-testid="confirm-password"]').attributes('data-has-error')).toBeDefined();
  });
  ...
  it('Change password success: POST called, fields cleared, positive Notify shown', async () => {
    ...
    await wrapper.find('[data-testid="current-password"]').setValue('oldpass');
    await wrapper.find('[data-testid="new-password"]').setValue('newpass99');
    await wrapper.find('[data-testid="confirm-password"]').setValue('newpass99');
    await wrapper.find('[data-testid="change-password-btn"]').trigger('click');
    await flushPromises();
    ...
    expect((wrapper.find('[data-testid="current-password"]').element as HTMLInputElement).value).toBe('');
    expect((wrapper.find('[data-testid="new-password"]').element as HTMLInputElement).value).toBe('');
    expect((wrapper.find('[data-testid="confirm-password"]').element as HTMLInputElement).value).toBe('');
    expect(vi.mocked(Notify.create)).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'positive', message: 'Password changed successfully.' }),
    );
  });

  it('Change password 400: negative Notify shown, fields not cleared', async () => {
    vi.mocked(api.post).mockRejectedValue(new ApiError(400, 'Current password is incorrect'));
    const wrapper = await mountResolved(false);
    await wrapper.find('[data-testid="current-password"]').setValue('wrongpass');
    await wrapper.find('[data-testid="new-password"]').setValue('newpass99');
    await wrapper.find('[data-testid="confirm-password"]').setValue('newpass99');
    await wrapper.find('[data-testid="change-password-btn"]').trigger('click');
    await flushPromises();
    expect(vi.mocked(Notify.create)).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'negative', message: 'Current password is incorrect' }),
    );
    expect((wrapper.find('[data-testid="current-password"]').element as HTMLInputElement).value).toBe('wrongpass');
  });
});
