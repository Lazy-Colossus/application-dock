...
describe("useApi", () => {
  it("parses 2xx JSON body for GET", async () => {
    mockFetch(
      async () =>
        new Response(JSON.stringify([{ id: "archery" }]), { status: 200 }),
    );
  ...
  it("throws ApiError with .status and .detail on 4xx", async () => {
    mockFetch(
      async () =>
        new Response(
          JSON.stringify({ detail: "A session is already in progress." }),
          {
            status: 409,
          },
        ),
  ...
  it("falls back to statusText when error body lacks detail", async () => {
    mockFetch(
      async () =>
        new Response("Server fell over", { status: 500, statusText: "BORK" }),
    );
  ...
  describe("401 redirect behaviour", () => {
    ...
    it("calls logout and redirects when token present and 401 on non-login endpoint", async () => {
      ...
      await expect(api.get("/archery/sessions")).rejects.toMatchObject({
        status: 401,
      });
    ...
    it("does NOT redirect when 401 on /auth/login (wrong credentials)", async () => {
      ...
      await expect(
        api.post("/auth/login", { username: "x", password: "y" }),
