...
- FR-3: Capability is decided by **which account you logged in as**: a username in the configured
  **guest list** is a guest, and **every other authenticated user is an admin**. A guest may read
  any calendar, claim a name and set their own votes, and nothing else.
...
The roster becomes real and personal. Admins add and remove invitees; removal keeps the past
intact and only clears the future. Anyone opening the calendar says which of
...
**When** it renders
**Then** it lists the active invitees in `order` as chips, and shows the add control only to
admins (FR-3).
...
active invitee as a row, each at least 44px tall. The month stays visible
...
### Story 2.4: Change your colour — ~~SUPERSEDED~~

**Withdrawn 2026-09-03.** Invitee colours were removed entirely: every place a
colour appeared, it sat beside the name it identified, so it was decoration rather
than information. This story built the recolour endpoint and the swatch UI, both of
which are deleted. Kept as a heading so the numbering of the epic is not silently
rewritten.
...
**Then** — ~~superseded~~; invitee colours were removed on 2026-09-03 and there is nothing to
pick. See the decision log, entry 33.
...
**Then** it lists them by the roster's `order`, by name (FR-13).
