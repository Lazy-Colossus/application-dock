...
4. Create `docs/stories/<app-name>/` with empty `for-review/` and `done/` subfolders.
...
`docs/` planning files (`architecture.md`, `prd.md`, `epics.md`, `ux/`) are **symlinks** into `_bmad-output/` — that directory holds the canonical planning artifacts. Code comments reference story IDs (e.g. "Story 6.1", "FR-2.8") as rationale for specific behaviors.
...
- **Draft** stories sit directly in the app folder root.
- **Ready for Review** stories move to `for-review/`.
- **Done** stories move to `done/`.
- Never place stories directly in `docs/stories/` root — always inside an app subfolder.
